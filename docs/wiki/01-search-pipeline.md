...
**Inside `ResearchBrandAsync`** — `ContextDevProfileResearcher` ([ContextDevProfileResearcher.cs](../../src/Daleel.Web/Profiles/ContextDevProfileResearcher.cs)):
1. **Real URL or nothing** — hostnames are never fabricated from the brand name (the old `GuessDomain` slug+`.com` heuristic is gone; a guessed domain that resolves scrapes some unrelated company's site). The caller's `siteUrlHint` (the saved profile's `Website`) is DNS/SSRF pre-flighted (`FirstUsableAsync`); brand-site *discovery* is owned by the enrichment queue's `BrandResearchHandler` (`BrandSiteActor`, latched by `Brand.SiteCheckedAt`), not this researcher. With a URL: `GatherBrandContextAsync` calls Context.dev `GetBrandAsync(domain)` (`GET /v1/brand/retrieve`) for name/description/industry, then scrapes the page (SSRF-guarded, capped 4000 chars). With no URL: zero provider calls — the synthesizer works from LLM knowledge alone.
...
1. **Real URL or nothing** — trust order: Google Places' website for the matched place (the Places lookup now runs *before* the scrape, so the same call that verifies the store also supplies its real site — the only reliable source for Arabic-named local stores) → the caller's saved-`Website` hint → `StoreSiteActor` discovery (search → read → confirm the store's own site; bounded to 20s, metered flows only). Every candidate is DNS/SSRF pre-flighted; with no real URL the scrape is skipped entirely. Hostnames are never fabricated from the store name.
...
3. **Google Maps cross-reference** — `FindPlaceAsync` (runs first, feeding step 1's trust order) + `StampPlaceDetails` (runs last):
   - `SearchPlacesAsync(storeName, profile.Center, radius 15000m, primaryLanguage)` (`POST /v1/places:searchText`).
   - Pick a **name-matching** place (`NameMatches`, accent/space-insensitive contains); a non-matching "closest result" is somebody else's shop, so no match = no verification.
   - Re-fetch full detail (`GetPlaceDetailsAsync`, the text-search mask omits hours/reviews).
   - **Stamp** `GooglePlaceId`, `GoogleMapsUrl`, `GoogleRating`, `GoogleReviewCount`, `Latitude/Longitude`, `OpeningHours`; backfill `Address`/`Phone`/`Website`/`Rating` only where empty. Best-effort — missing key / no confident match leaves the profile un-verified.
