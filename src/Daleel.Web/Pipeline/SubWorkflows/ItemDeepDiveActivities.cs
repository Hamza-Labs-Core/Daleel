...
public sealed class IdentifyProductActivity : CodeActivity
{
    protected override async ValueTask ExecuteAsync(ActivityExecutionContext context)
    {
        ...
        try
        {
            ...
            state.Log($"Identified {state.Model.Name} as “{id.CanonicalModelName}” ({id.Method}, {id.Confidence:P0}).");
...
public sealed class MergeAndCleanSpecsActivity : CodeActivity
{
    protected override ValueTask ExecuteAsync(ActivityExecutionContext context)
    {
        ...
        state.Log($"Merged {state.RawSpecsBySource.Count} source(s) into {merged.Count} canonical spec(s) for {state.Model.Name}.");
...
public sealed class SaveFinalSpecsActivity : CodeActivity
{
    ...
    protected override async ValueTask ExecuteAsync(ActivityExecutionContext context)
    {
        ...
        state.Log($"Saved canonical spec sheet for {state.Model.Name} ({merged.Count} spec(s)).");
