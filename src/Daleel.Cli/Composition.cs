...
internal static class Composition
{
    ...
    /// <summary>Selects an LLM client, preferring Anthropic, then OpenAI.</summary>
    public static ILlmClient BuildLlm()
    {
        ...
        throw new InvalidOperationException("Set ANTHROPIC_API_KEY or OPENAI_API_KEY to use the agent.");
    }

    /// <summary>Builds a fully-wired <see cref="AgentService"/> from the environment.</summary>
    public static AgentService BuildAgent(string defaultGeo, Action<string>? log = null)
    {
        var llm = BuildLlm();
